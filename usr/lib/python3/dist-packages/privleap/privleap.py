...
class PrivleapSession:
    ...
    # attack.
    def __init__(self, session_info: str | socket.socket, user_name: str = "", is_control_session: bool = False):
    ...
    def __recv_msg(self) -> bytes:
        ...
        while len(recv_buf) != header_len:
            try:
                tmp_buf: bytes = self.backend_socket.recv(header_len - len(recv_buf))
    ...
    @staticmethod
    def __parse_msg_parameters(recv_buf: bytes, str_count: int, blob_at_end: bool) -> Tuple[list[str], bytes | None]:
    ...
    def get_msg(self) -> PrivleapMsg:
        ...
        if self.is_control_session and self.is_server_side:
            if msg_type_str == "CREATE":
                param_list, _ = self.__parse_msg_parameters(recv_buf, str_count = 1, blob_at_end = False)
        ...
        # Client-side comm socket, we're receiving, so expect server comm messages
            ...
            if msg_type_str == "TRIGGER":
                ...
                param_list, blob = self.__parse_msg_parameters(recv_buf, str_count = 1, blob_at_end = True)
                ...
                param_list, _ = self.__parse_msg_parameters(recv_buf, str_count = 2, blob_at_end = False)
    ...
    def __send_msg(self, msg_obj: PrivleapMsg) -> None:
        ...
        while msg_payload_sent < len(msg_payload):
            msg_sent: int = self.backend_socket.send(msg_payload[msg_payload_sent:])
            if msg_sent == 0:
                raise ConnectionAbortedError("Connection unexpectedly closed")
...
class PrivleapCommon:
    ...
    user_name_regex: re.Pattern[str] = re.compile(r"[A-Za-z_][-A-Za-z0-9_]*\$?\Z")
    signal_name_regex: re.Pattern[str] = re.compile(r"[-A-Za-z0-9_.]+\Z")

    @staticmethod
    def validate_id(id_string: str, validate_type: PrivleapValidateType) -> bool:
